...
var color = d3.scale.linear()
  ...
  function row(row) {
    var cell = d3.select(this).selectAll(".cell")
      ...
      .enter().append("rect")
        ...
        .style("fill", function(d) { return color(d.z/2); })// nodes[d.x].group == nodes[d.y].group ? c(nodes[d.x].group) : null; })
  ...
  function mouseover(p) {
    ...
    d3.selectAll(".column text").classed("active", function(d, i) { return i == p.x; });
      d3.select(this).insert("title").text(p.z+': '+nodes[p.y].name + "--" + nodes[p.x].name);
