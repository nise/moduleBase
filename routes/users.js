			...
			Users.remove({}, function(err) { 
				...
				csv().from.string(data, {comment: '#'} )
					.to.array( function(data){
						...
						for(var i = 1; i < data.length; i++){ console.log(data[i])
							new Users({
								id: i,
								firstname: data[i][0],
								name : data[i][1],
								username: data[i][2],// === '' ? (data[i][0]).replace(/\ /g,'.').toLowerCase() + '.' + (data[i][1]).toLowerCase() : data[i][14],//data[i][13], 
								password: data[i][3],
								hs: data[i][4],
								email: data[i][5],
								role: data[i][6],							
...
exports.renderIndex = function(req, res) {
  Users.find().sort( 'username' ).lean().exec(function (err, items) {
	  if(err){ 
			...
			res.render('admin/users-index', {items: items, user: req.user}); 
			res.end('done');
		}	 
  });
};
